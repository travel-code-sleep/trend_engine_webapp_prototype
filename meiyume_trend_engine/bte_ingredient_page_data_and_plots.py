...
ing_page_source_options = [{'label': i, 'value': i}
                           for i in ing_page_ing_df.source.unique()]
ing_page_category_options = [{'label': i, 'value': i}
                             for i in ing_page_ing_df.category.unique()]
ing_page_product_type_options = [{'label': i, 'value': i}
                                 for i in ing_page_ing_df.product_type.unique()]
ing_page_ingredient_options = [{'label': i, 'value': i}
                               for i in ing_page_ing_df.ingredient.unique()]

""" create graph figure functions"""


def create_ing_page_ingredient_type_figure(source: str, category: str,
                                           product_type: str) -> go.Figure:
    ...
    """
    data = pd.DataFrame(ing_page_ing_df[(ing_page_ing_df.source == source) &
                                        (ing_page_ing_df.category == category) &
                                        (ing_page_ing_df.product_type == product_type)
                                        ][['ingredient', 'ingredient_type']
                                          ].drop_duplicates(subset='ingredient').ingredient_type.value_counts()).reset_index()
    ...
    fig = px.bar(data, x="count", y='ingredient_type', width=900, height=300, orientation='h',
